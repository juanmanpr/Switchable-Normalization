# ------------------------------------------------------------------------------
# Based on code by Bin Xiao (Bin.Xiao@microsoft.com)
# ------------------------------------------------------------------------------
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, ibn=True):
        ...
        if ibn:
            print('Using IBN')
            self.bn1 = IBN(planes, momentum=BN_MOMENTUM)    
        else:
            self.bn1 = nn.BatchNorm2d(planes, momentum=BN_MOMENTUM)
...
class HighResolutionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        y = torch.nn.functional.avg_pool2d(y, kernel_size=y.size()
                             [2:]).view(y.size(0), -1)
...
def hrnet_ibna_big(**kwargs):
    ...
    model = HighResolutionNet(cfg, num_classes=1000, pretrained_layers = pretrained_layers)
...
def get_pose_net(cfg, is_train, **kwargs):
    ...
    if is_train and cfg.MODEL.INIT_WEIGHTS:
        try:
            ...
            import warnings
            warnings.warn('Error in loading pretrained weights, '
                          'please check if the weights are available at "{}"'.format(cfg.MODEL.PRETRAINED))
